...
// See: https://forums.xbsx2.net/Thread-blog-PS2-VU-Vector-Unit-Documentation-Part-1
...
__ri int mVUbranchCheck(mV)
{
	...
	if (isEvilBlock)
	{
		...
		{
			Console.Error("microVU%d: %s in branch, branch delay slot requires link [%04x] - If game broken report to XBSX2 Team", mVU.index,
				branchSTR[mVUlow.branch & 0xf], xPC);
		}
		else
		{
			DevCon.Warning("microVU%d: %s in branch, branch delay slot! [%04x] - If game broken report to XBSX2 Team", mVU.index,
				branchSTR[mVUlow.branch & 0xf], xPC);
		}
		return 1;
	}
	...
	if (mVUlow.branch)
	{
		...
		if (doBranchInDelaySlot)
		{
			...
			mVUregs.needExactMatch |= 7; // This might not be necessary, but w/e...
			mVUregs.flagInfo = 0;
			DevCon.Warning("microVU%d: %s in %s delay slot! [%04x]  - If game broken report to XBSX2 Team", mVU.index,
