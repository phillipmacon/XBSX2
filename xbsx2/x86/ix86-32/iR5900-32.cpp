...
void recompileNextInstruction(int delayslot)
{
	...
	if (delayslot)
	{
		...
		// Original PR and discussion at https://github.com/XBSX2/xbsx2/pull/1783 so we don't forget this information.
	...
	if (_Opcode_ == 022) // COP2
	{
		...
		else if ((cpuRegs.code & 0x7FC) == 0x3BC) // DIV/RSQRT/SQRT/WAITQ
		{
			...
			for (u32 p = pc; cycles > 0 && p < s_nEndBlock; p += 4, cycles--)
			{
				...
				{
					std::string disasm;
					Console.Warning("Possible incorrect Q value used in COP2. If the game is broken, please report to http://github.com/xbsx2/xbsx2.");
		...
		else
		{
			...
			for (u32 p = pc; s != 0 && p < s_nEndBlock && all_count < 10 && cop2m_count < 5 && cop2o_count < 4; p += 4)
			{
				...
				if (_Opcode_ == 022 && _Rs_ == 2) // CFC2
					...
					{
						std::string disasm;
						Console.Warning("Possible old value used in COP2 code. If the game is broken, please report to http://github.com/xbsx2/xbsx2.");
						for (u32 i = s_pCurBlockEx->startpc; i < s_nEndBlock; i += 4)
						{
							disasm = "";
							disR5900Fasm(disasm, memRead32(i), i, false);
...
void LoadAllPatchesAndStuff(const Xbsx2Config&);
	...
	if (g_eeloadMain && HWADDR(startpc) == HWADDR(g_eeloadMain))
	{
		...
		if (g_SkipBiosHack)
		{
			...
			if ((typeBexecjump >> 26 == 3) || (typeCexecjump >> 26 == 3) || (typeDexecjump >> 26 == 3)) // JAL to 0x822B8
				...
				Console.WriteLn("recRecompile: Could not enable launch arguments for fast boot mode; unidentified BIOS version! Please report this to the XBSX2 developers.");
		}
