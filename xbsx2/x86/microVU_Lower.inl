...
void condEvilBranch(mV, int JMPcc)
{
	...
	else
	{
		...
		incPC(-2);
		if (mVUlow.branch >= 9)
			DevCon.Warning("Conditional in JALR/JR delay slot - If game broken report to XBSX2 Team");
		incPC(2);
	}
}
...
mVUop(mVU_BAL)
{
	...
	pass2
	{
		...
		{
			incPC(-2);
			DevCon.Warning("Linking BAL from %s branch taken/not taken target! - If game broken report to XBSX2 Team", branchSTR[mVUlow.branch & 0xf]);
			incPC(2);
			if (isEvilBlock)
...
mVUop(mVU_JALR)
{
	...
	pass2
	{
		...
		if (mVUlow.evilBranch)
		{
			...
			{
				incPC(-2);
				DevCon.Warning("Linking JALR from %s branch taken/not taken target! - If game broken report to XBSX2 Team", branchSTR[mVUlow.branch & 0xf]);
				incPC(2);
