	...
	: NetAdapter()
{
	...
	if (pcap_io_init(EmuConfig.DEV9.EthDevice, EmuConfig.DEV9.EthApi == Xbsx2Config::DEV9Options::NetApi::PCAP_Switched, newMAC) == -1)
	{
...
std::vector<AdapterEntry> PCAPAdapter::GetAdapters()
{
	...
	{
		AdapterEntry entry;
		entry.type = Xbsx2Config::DEV9Options::NetApi::PCAP_Switched;
#ifdef _WIN32
		//guid
		...
		nic.push_back(entry);
		entry.type = Xbsx2Config::DEV9Options::NetApi::PCAP_Bridged;
		nic.push_back(entry);
		d = d->next;
	}
