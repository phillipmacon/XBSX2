...
static void _vu1Exec(VURegs* VU)
{
	...
	else
	{
		...
		if (uregs.VFwrite)
		{
			...
			{
				//Console.Warning("*XBSX2*: Warning, VF write to the same reg in both lower/upper cycle pc=%x", VU->VI[REG_TPC].UL);
				discard = 1;
			}
		...
		if (uregs.VIwrite & (1 << REG_CLIP_FLAG))
		{
			...
			{
				//Console.Warning("*XBSX2*: Warning, VI write to the same reg in both lower/upper cyclepc=%x", VU->VI[REG_TPC].UL);
				discard = 1;
			}
			if (lregs.VIread & (1 << REG_CLIP_FLAG))
			{
				//Console.Warning("*XBSX2*: Warning, VI read same cycle as write pc=%x", VU->VI[REG_TPC].UL);
