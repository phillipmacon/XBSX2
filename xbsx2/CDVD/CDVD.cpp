...
static std::string ExecutablePathToSerial(const std::string& path)
{
	...
	// this maintains the old behavior of XBSX2.
...
void cdvdReloadElfInfo(std::string elfoverride)
{
	...
	try
	{
		...
		if (discType == 1)
		{
			// XBSX2 currently only recognizes *.elf executables in proper PS2 format.
...
static __fi void cdvdWrite07(u8 rt) // BREAK
{
	...
	DbgCon.WriteLn("*XBSX2*: CDVD BREAK %x", rt);
...
{ // TYPE
	...
	DevCon.WriteLn("*XBSX2*: CDVD TYPE %x", rt);
}
...
static __fi void cdvdWrite14(u8 rt)
{
	...
	if (rt == 0xFE)
		Console.Warning("*XBSX2*: Unimplemented PS1 mode DISC SPEED = FAST");
	else
		Console.Warning("*XBSX2*: Unimplemented PS1 mode DISC SPEED = STANDARD");
}
...
{ // SDATAOUT
	...
	Console.WriteLn("*XBSX2* SDATAOUT");
}
