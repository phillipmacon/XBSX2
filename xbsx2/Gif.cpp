...
void GIFdma()
{
	while (gifch.qwc > 0 || !gif.gspath3done)
	{
		...
		{
			Console.WriteLn("GIF DMA Stall in Normal mode not implemented - Report which game to XBSX2 Team");
		}
...
void mfifoGIFtransfer()
{
	...
	if (gifch.qwc == 0)
	{
		...
		{
			Console.WriteLn("GIF MFIFO DMA Stall not implemented - Report which game to XBSX2 Team");
		}
