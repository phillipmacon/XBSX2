...
// distinctly different values for most fields when read and written.  In XBSX2 we house
// the written version in the gsRegs buffer, and generate the readback version on-demand
// from various other XBSX2 system statuses.
union tGS_CSR
{
	struct
	{
		...
		//       (XBSX2 implementation also clears GIFpaths, though that behavior may differ on real HW).
		// Read: Always returns 0. (?)
		u64 RESET : 1;
...
// by the XBSX2 core library.  It provides an interface for the linking User Interface
...
extern void gsUpdateFrequency(Xbsx2Config& config);
