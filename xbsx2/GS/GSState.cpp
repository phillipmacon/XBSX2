...
void GSState::UpdateSettings(const Xbsx2Config::GSOptions& old_config)
{
	m_mipmap = GSConfig.Mipmap;
...
float GSState::GetTvRefreshRate()
{
	...
	switch (videomode)
	{
		...
		case GSVideoMode::HDTV_1080I:
			return 60;
		default:
			Console.Error("GS: Unknown video mode. Please report: https://github.com/XBSX2/xbsx2/issues");
			return 0;
	}
...
void GSState::Transfer(const u8* mem, u32 size)
{
	...
	if (index == 0)
	{
		if (size == 0 && path.nloop > 0)
		{
			...
			// having an EOP specified anywhere in VU1 memory.  Needed until XBSX2 is fixed to
...
GIFRegTEX0 GSState::GetTex0Layer(u32 lod)
{
	...
	switch (lod)
	{
		...
		case 6:
			...
			break;
		default:
			Console.Error("GS: Invalid guest lod setting. Please report: https://github.com/XBSX2/xbsx2/issues");
	}
