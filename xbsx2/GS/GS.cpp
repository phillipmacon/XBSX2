...
#include "xbsx2/Config.h"
#include "xbsx2/Counters.h"
...
Xbsx2Config::GSOptions GSConfig;
...
int GSinit()
{
	...
	// Vector instructions must be avoided when initialising GS since XBSX2
...
bool GSreopen(bool recreate_display, const Xbsx2Config::GSOptions& old_config)
{
...
bool GSopen(const Xbsx2Config::GSOptions& config, GSRendererType renderer, u8* basemem)
{
...
void GSUpdateConfig(const Xbsx2Config::GSOptions& new_config)
{
	Xbsx2Config::GSOptions old_config(std::move(GSConfig));
	GSConfig = new_config;
...
void GSSwitchRenderer(GSRendererType new_renderer)
{
	...
	const Xbsx2Config::GSOptions old_config(GSConfig);
...
BEGIN_HOTKEY_LIST(g_gs_hotkeys)
	...
	{"CycleAspectRatio", "3. Graphics", "Cycle Aspect Ratio", [](s32 pressed) {
		 ...
		 Host::AddKeyedFormattedOSDMessage("CycleAspectRatio", 10.0f, "Aspect ratio set to '%s'.", Xbsx2Config::GSOptions::AspectRatioNames[static_cast<int>(EmuConfig.CurrentAspectRatio)]);
	 }},
