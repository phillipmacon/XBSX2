...
void FolderMemoryCard::Open(std::string fullPath, const Xbsx2Config::McdOptions& mcdOptions, const u32 sizeInClusters, const bool enableFiltering, std::string filter, bool simulateFileWrites)
{
...
void FolderMemoryCard::LoadMemoryCardData(const u32 sizeInClusters, const bool enableFiltering, const std::string& filter)
{
	...
	const std::string superBlockFileName(Path::Combine(m_folderName, "_xbsx2_superblock"));
	...
	if (FileSystem::DirectoryExists(dirPath.c_str()))
	{
		...
		for (const auto& file : GetOrderedFiles(dirPath))
		{
			...
			else
			{
				...
				// set metadata
				const std::string metaFileName(Path::Combine(Path::Combine(dirPath, "_xbsx2_meta_directory"), file.m_fileName));
...
bool FolderMemoryCard::AddFile(MemoryCardFileEntry* const dirEntry, const std::string& dirPath, const EnumeratedFileEntry& fileEntry, MemoryCardFileMetadataReference* parent)
{
	...
	if (auto file = FileSystem::OpenManagedCFile(filePath.c_str(), "rb"); file)
	{
		...
		std::string metaFileName(Path::Combine(Path::Combine(dirPath, "_xbsx2_meta"), fileEntry.m_fileName));
		if (auto metaFile = FileSystem::OpenManagedCFile(metaFileName.c_str(), "rb"); metaFile)
		{
...
u32 FolderMemoryCard::CalculateRequiredClustersOfDirectory(const std::string& dirPath) const
{
	...
	{
		if (StringUtil::StartsWith(fd.FileName, "_xbsx2_"))
			continue;
			...
			if (entry->IsDir())
			{
				if (!entry->IsDotDir())
				{
					...
					if (m_performFileWrites)
					{
						...
						std::string metaFileName(Path::Combine(fullSubDirPath, "_xbsx2_meta_directory"));
						...
						metaFileName = Path::Combine(fullSubDirPath, "_xbsx2_index");
						std::optional<ryml::Tree> yaml = loadYamlFile(metaFileName.c_str());

						// if _xbsx2_index hasn't been made yet, start a new file
						if (!yaml.has_value())
						{
	...
	for (auto it = oldFileEntries.cbegin(); it != oldFileEntries.cend(); ++it)
	{
		...
		if (entry->IsValid() && entry->IsUsed() && !entry->IsDotDir())
		{
			...
			if (newEntry == nullptr)
			{
				...
				const std::string newFilePath(Path::Combine(Path::Combine(m_folderName, dirPath), fmt::format("_xbsx2_deleted_{}", cleanName)));
...
void FolderMemoryCard::AttemptToRecreateIndexFile(const std::string& directory) const
{
	...
	FileSystem::CopyFilePath(Path::Combine(directory, "_xbsx2_index").c_str(),
		Path::Combine(directory, "_xbsx2_index.invalid.bak").c_str(), true);
	...
	{
		if (fd.FileName.rfind("_xbsx2_", 0) == 0)
		{
			continue;
		}
	...
	auto file = FileSystem::OpenManagedCFile(Path::Combine(directory, "_xbsx2_index").c_str(), "w");
	if (file)
...
std::vector<FolderMemoryCard::EnumeratedFileEntry> FolderMemoryCard::GetOrderedFiles(const std::string& dirPath) const
{
	...
	if (!results.empty())
	{
		...
		for (FILESYSTEM_FIND_DATA& fd : results)
		{
			...
			if (!(fd.Attributes & FILESYSTEM_FILE_ATTRIBUTE_DIRECTORY))
			{
				std::optional<ryml::Tree> yaml = loadYamlFile(Path::Combine(dirPath, "_xbsx2_index").c_str());
			...
			else
			{
				...
				std::string subDirIndexPath(Path::Combine(subDirPath, "_xbsx2_index"));
...
{
	const std::string indexName(Path::Combine(filePath, "_xbsx2_index"));
...
void FileAccessHelper::WriteMetadata(const std::string_view& folderName, const MemoryCardFileMetadataReference* fileRef)
{
	...
	std::string metaFileName(Path::AppendDirectory(fileName, "_xbsx2_meta"));
...
void FileAccessHelper::WriteIndex(const std::string& baseFolderName, MemoryCardFileEntry* const entry, MemoryCardFileMetadataReference* const parent)
{
	...
	const std::string indexFileName(Path::Combine(folderName, "_xbsx2_index"));
