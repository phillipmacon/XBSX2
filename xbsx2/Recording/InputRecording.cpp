...
void SaveStateBase::InputRecordingFreeze()
{
	...
	if (g_Conf->EmuOptions.EnableRecordingTools)
	{
		...
		// that starts from a savestate (not power-on) and the starting (xbsx2 internal) frame
...
void InputRecording::RecordingReset()
{
	...
	// that starts from power-on and the starting (xbsx2 internal) frame
...
void InputRecording::SetFrameCounter(u32 newGFrameCount)
{
	...
	{
		inputRec::consoleLog("Warning, you've loaded XBSX2 emulation to a point after the end of the original recording. This should be avoided.");
	...
	else
	{
		...
		{
			inputRec::consoleLog("Warning, you've loaded XBSX2 emulation to a point before the start of the original recording. This should be avoided.");
			if (state == InputRecordingMode::Recording)
				SetToReplayMode();
		}
...
void InputRecording::SetupInitialState(u32 newStartingFrame)
{
	...
	else
	{
		...
		inputRec::consoleMultiLog({fmt::format("File: {}", inputRecordingData.GetFilename().ToUTF8()),
			fmt::format("XBSX2 Version Used: {}", std::string(inputRecordingData.GetHeader().emu)),
...
{
	inputRec::consoleLog(fmt::format("{} is not compatible with this version of XBSX2", savestate.ToUTF8()));
	inputRec::consoleLog(fmt::format("Original XBSX2 version used: {}", inputRecordingData.GetHeader().emu));
...
void InputRecording::FailedSavestate()
{
	inputRec::consoleLog(fmt::format("{} is not compatible with this version of XBSX2", savestate));
	inputRec::consoleLog(fmt::format("Original XBSX2 version used: {}", inputRecordingData.GetHeader().emu));
	inputRecordingData.Close();
	initialLoad = false;
	state = InputRecordingMode::NotActive;
