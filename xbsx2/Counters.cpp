...
u32 UpdateVSyncRate()
{
	...
	case GS_VideoMode::PAL:
	case GS_VideoMode::DVD_PAL:
		custom = (EmuConfig.GS.FrameratePAL != Xbsx2Config::GSOptions::DEFAULT_FRAME_RATE_PAL);
		if (gsIsInterlaced)
	...
	case GS_VideoMode::NTSC:
	case GS_VideoMode::DVD_NTSC:
		custom = (EmuConfig.GS.FramerateNTSC != Xbsx2Config::GSOptions::DEFAULT_FRAME_RATE_NTSC);
		if (gsIsInterlaced)
			total_scanlines = SCANLINES_TOTAL_NTSC_I;
		else
			total_scanlines = SCANLINES_TOTAL_NTSC_NI;
	...
	default:
		if (gsIsInterlaced)
			...
			total_scanlines = SCANLINES_TOTAL_NTSC_NI;
		Console.Error("XBSX2-Counters: Unknown video mode detected");
