...
s32 SPU2Savestate::FreezeIt(DataBlock& spud)
{
	...
	pxAssertMsg(spu2regs && _spu2mem, "Looks like XBSX2 is trying to savestate while components are shut down.  That's a no-no! It shouldn't crash, but the savestate will probably be corrupted.");
	...
	// note: Don't save the cache.  XBSX2 doesn't offer a safe method of predicting
...
s32 SPU2Savestate::ThawIt(DataBlock& spud)
{
	if (spud.spu2id != SAVE_ID || spud.version < SAVE_VERSION)
	{
		...
		if (spud.spu2id == SAVE_ID)
			fprintf(stderr, "\tSavestate version is from an older version of XBSX2.\n");
		else
	...
	else
	{
		...
		pxAssertMsg(spu2regs && _spu2mem, "Looks like XBSX2 is trying to loadstate while components are shut down.  That's a no-no!  It shouldn't crash, but the savestate will probably be corrupted.");
		...
		// editions of XBSX2 with fixed addressing, but new XBSX2s have dynamic memory
		// addressing).
