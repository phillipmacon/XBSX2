...
static void _vu0Exec(VURegs* VU)
{
	...
	else
	{
		...
		if (uregs.VFwrite)
		{
			...
			{
				//				Console.Warning("*XBSX2*: Warning, VF write to the same reg in both lower/upper cycle");
				discard = 1;
			}
		...
		if (uregs.VIread & (1 << REG_CLIP_FLAG))
		{
			...
			{
				//Console.Warning("*XBSX2*: Warning, VI write to the same reg in both lower/upper cycle");
				discard = 1;
			}
