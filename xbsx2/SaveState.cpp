...
static const char* EntryFilename_StateVersion = "XBSX2 Savestate Version.id";
static const char* EntryFilename_Screenshot = "Screenshot.png";
static const char* EntryFilename_InternalStructures = "XBSX2 Internal Structures.dat";
...
//  block, since they're all XBSX2-dependent and having separate files in the archie for them
//  would not be useful).
//
...
static void CheckVersion(const std::string& filename, zip_t* zf)
{
	...
	if (!zff || zip_fread(zff.get(), &savever, sizeof(savever)) != sizeof(savever))
	{
		throw Exception::SaveStateLoadError(filename)
			...
			.SetUserMsg("This file is not a valid XBSX2 savestate.  See the logfile for details.");
	}
...
void SaveState_UnzipFromDisk(const std::string& filename)
{
	...
	if (!zf)
	{
		...
		throw Exception::SaveStateLoadError(filename)
			...
			.SetUserMsg("This savestate cannot be loaded because it is not a valid gzip archive.  It may have been created by an older unsupported version of XBSX2, or it may be corrupted.");
	}
