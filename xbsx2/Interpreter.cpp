...
static void execI()
{
	...
	// Extra note: due to some cycle count issue XBSX2's internal debugger is
	// not yet usable with the interpreter
//#define EXTRA_DEBUG
#if defined(EXTRA_DEBUG) || defined(XBSX2_DEVBUILD)
					...
					else if (cpuRegs.pc == g_eeloadMain)
					{
						...
						if (g_SkipBiosHack)
						{
							...
							if ((typeBexecjump >> 26 == 3) || (typeCexecjump >> 26 == 3) || (typeDexecjump >> 26 == 3)) // JAL to 0x822B8
								...
								Console.WriteLn("intExecute: Could not enable launch arguments for fast boot mode; unidentified BIOS version! Please report this to the XBSX2 developers.");
						}
					}
