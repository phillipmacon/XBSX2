...
*/
...
#elif !defined(XBSX2_APP_DATADIR)
		return AppRoot();
#else
		// Each linux distributions have his rules for path so we give them the possibility to
		// change it with compilation flags. -- Gregory
		return wxDirName(XBSX2_APP_DATADIR).MakeAbsolute(AppRoot().ToString());
#endif
	}
	...
	wxDirName GetDocs()
	{
#if !defined(XBSX2_APP_DOCDIR)
		return AppRoot() + Base::Docs();
#else
		return wxDirName(XBSX2_APP_DOCDIR).MakeAbsolute(AppRoot().ToString());
#endif
	}
