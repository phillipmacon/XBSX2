...
bool X11NoGUIPlatform::Initialize()
{
	...
	if (res == 0)
		Console.Warning("XInitThreads() returned %d, things might not be stable.", res);

	m_display = XOpenDisplay(nullptr);
	if (!m_display)
	{
		Console.Error("Failed to connect to X11 display.");
		return false;
	}
...
bool X11NoGUIPlatform::CreatePlatformWindow(std::string title)
{
	...
	XDisplayLocker locker(m_display);
	{
		...
		if (!m_window)
		{
			Console.Error("Failed to create X window");
			return false;
		}
...
void X11NoGUIPlatform::SetFullscreen(bool enabled)
{
	...
	if (!XSendEvent(m_display, DefaultRootWindow(m_display), False, SubstructureRedirectMask | SubstructureNotifyMask,
			&event))
	{
		Console.Error("Failed to switch to %s", enabled ? "Fullscreen" : "windowed");
		return;
	}
...
{
	Console.Error("X11NoGUIPlatform::OpenURL() not implemented: %.*s", static_cast<int>(url.size()), url.data());
	return false;
}

bool X11NoGUIPlatform::CopyTextToClipboard(const std::string_view& text)
{
	Console.Error("X11NoGUIPlatform::CopyTextToClipboard() not implemented: %.*s", static_cast<int>(text.size()), text.data());
	return false;
}
